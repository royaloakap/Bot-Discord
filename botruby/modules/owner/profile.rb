      ...
      code: proc { |event, args|
        ...
        when 'nick', 'nickname'
          begin
            ...
            event.respond("#{royaloakap.config['emoji_tickbox']} Reset nickname!")
          rescue Discordrb::Errors::NoPermission
            error = royaloakap.config['emoji_error']
            event.respond("#{error} I don't have permission to change my nickname!")
          end
        when 'avatar', 'avy'
          ...
          event.respond "#{royaloakap.config['emoji_success']} Reset avatar!"
        when 'game', 'playing'
          event.bot.game = nil
          event.respond("#{royaloakap.config['emoji_tickbox']} Reset game!")
        else
          event.respond("#{royaloakap.config['emoji_error']} Enter a valid subcommand!\n" \
